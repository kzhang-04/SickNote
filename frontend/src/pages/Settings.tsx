...
const Settings = () => {
    ...
    const [notificationPrivacy, setNotificationPrivacy] =
    ...
    useEffect(() => {
        // if not logged in, don't call backend
        if (!token) {
            setStatusMessage("You must be logged in to view or change settings.");
            setIsError(true);
            return;
        }
        ...
        const fetchCurrentSetting = async () => {
            try {
                ...
                const response = await fetch(
                    `${API_BASE_URL}/api/settings/privacy`,
                    {
                        headers: {
                ...
                if (response.ok) {
                    ...
                    setNotificationPrivacy(
                        data.notification_privacy as PrivacyOption
                    );
                } else {
                    setStatusMessage(
                        "Could not load settings from server. (Check server logs)"
                    );
                    setIsError(true);
                }
            } catch {
                setStatusMessage(
                    "Network error during settings load. Is the backend server running?"
                );
                setIsError(true);
            } finally {
                setIsLoading(false);
        ...
        void fetchCurrentSetting();
    }, [token]);

    const handleSaveSettings = async (newValue: PrivacyOption) => {
        // if somehow clicked without token
        if (!token) {
            setStatusMessage("You must be logged in to change settings.");
            setIsError(true);
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                setStatusMessage(
                    `Settings saved successfully! Current setting: ${data.setting}`
                );
            } else {
        ...
        <div className="min-h-screen">
            <div className="container mx-auto px-4 py-8 max-w-lg">
                ...
                <div className="bg-card p-6 rounded-lg border border-border shadow-lg">
                    <h2 className="text-xl font-semibold mb-4 border-b pb-2 text-foreground">
                        Notification Privacy
                    </h2>

                    {!token && (
                        <p className="text-sm text-red-600 mb-4">
                            You must be logged in to change your notification privacy.
                        </p>
                    )}

                    <fieldset className="space-y-4" disabled={!token || isLoading}>
                            ...
                            >
                                <div className="flex items-center h-5">
                                    <input
                                        ...
                                        onChange={(e) =>
                                            handleSaveSettings(
                                                e.target.value as PrivacyOption
                                            )
                                        }
                    ...
                    <p className="mt-6 text-xs text-muted-foreground">
                        <strong>Current Persistent Setting:</strong>{" "}
                        <span className="font-semibold text-primary">
                            {notificationPrivacy.toUpperCase()}
                        </span>
                    </p>
                </div>
            </div>
